...
class CenterHead(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        if self.training:
            ...
            # data_dict['svd_masks'] = targets_dict["svd_masks"]
            # targets_dict.pop('svd_masks', None)
    ...
    def assign_targets(self, gt_boxes):
        ...
        # heatmaps, anno_boxes, inds, masks, svd_masks = multi_apply(
        ...
        # svd_masks = np.array(svd_masks).transpose(1, 0).tolist()
        # svd_masks = [torch.stack(svd_masks) for svd_masks in svd_masks]
        # svd_masks = torch.cat(svd_masks)
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d):
        ...
        grid_size = torch.tensor(self.grid_size).to(device)
        pc_range = torch.tensor(self.point_cloud_range)
        voxel_size = torch.tensor(self.target_cfg.VOXEL_SIZE)

        feature_map_size = (
            grid_size[:2] // self.target_cfg.OUT_SIZE_FACTOR).to(device)
        ...
        # heatmaps, anno_boxes, inds, masks, svd_masks = [], [], [], [], []
        heatmaps, anno_boxes, inds, masks = [], [], [], []
        for idx in range(1):
            heatmap = gt_bboxes_3d.new_zeros(
                (len(self.class_names[idx]), feature_map_size[1],
                 feature_map_size[0]))
            # svd_mask = gt_bboxes_3d.new_zeros(
            #     (len(self.class_names[idx]), grid_size[1],
            ...
            for k in range(num_objs):
                ...
                # width_svd = width / voxel_size[0]
                # length_svd = length / voxel_size[1]
                ...
                if width > 0 and length > 0:
                    ...
                    # bbox = torch.tensor([max(coor_x_svd - width_svd / 2, 0), min(coor_x_svd + width_svd / 2, grid_size[0]),
                    #                     max(coor_y_svd - length_svd / 2, 0), min(coor_y_svd + length_svd / 2, grid_size[1])],
                    #                     dtype=torch.float32,
                    #                     device=device)
                    # bbox_int = bbox.to(torch.int32)
                    # svd_mask[cls_id, bbox_int[2]: bbox_int[3],
            ...
            masks.append(mask)
            inds.append(ind)
            # svd_masks.append(svd_mask)
        return heatmaps, anno_boxes, inds, masks
        # return heatmaps, anno_boxes, inds, masks, svd_masks
    ...
    def generate_predicted_boxes(self, batch_size, cls_preds, box_preds, dir_cls_preds=None):
        ...
        ys, xs = torch.meshgrid(
            [torch.arange(0, H, device=cls_preds.device), torch.arange(0, W, device=cls_preds.device)])

        ys = ys.to(cls_preds.device).view(1, H, W).repeat(batch, 1, 1)
        xs = xs.to(cls_preds.device).view(1, H, W).repeat(batch, 1, 1)
